...
def get_kamstrup_param_meta():
    """
    Get Kamstrup parameter metadata for Home Assistant.
    
    This is loaded from the Home Assistant extension module, which is optional
    and only imported when Home Assistant integration is needed.
    
    Returns:
        dict: Parameter metadata mapping for Home Assistant integration
    
    Raises:
        ImportError: If the Home Assistant extension is not available
    """
    try:
        from .extensions.ha_metadata import get_param_meta
        return get_param_meta()
    except ImportError as e:
        raise ImportError(
            "Home Assistant metadata extension not available. "
            "Ensure kamstrup2mqtt.extensions.ha_metadata is installed."
        ) from e
